...
server = app.server
# Clear the layout and do not display exceptions till the callback gets executed
...
def update_graphs(n_clicks, zone, count_type, prediction_interval):
    try:
        ...
        # Check if any error occurred in the get_predictions function
        if trend is None:
            raise Exception("Unable to generate predictions.")
        ...
        error_message = f"An error occurred: {str(e)}"
        error_graph = go.Figure()
        error_graph.update_layout(title="Error",
                                  xaxis_title="",
                                  yaxis_title="",
                                  showlegend=False,
                                  plot_bgcolor="rgba(0, 0, 0, 0)",
        ...
        return error_graph, error_graph, error_graph, error_graph
